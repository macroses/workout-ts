...
<template>
  <div
    class="workout-data"
    :class="{ active: store.readWorkout && !store.isEditMode }"
  >
    <div
      class="workout-data__header"
      :style="{ backgroundColor: `rgb(${store.readWorkout?.color})` }"
    >
      <div class="workout-data__date">
        {{
          dayjs
            .unix(store.readWorkout?.workoutDate?.seconds)
            .format("DD.MM.YYYY")
        }}
      </div>
      <div class="workout-data__funcs">
        ...
        <div
          class="a11y-wrap"
          @click="deleteWorkoutAndCloseReadModal(store.readWorkout?.id)"
        >
      ...
      <ul class="workout-data__list">
        ...
        >
          ...
          <ul class="workout-data__sets-list" v-if="exerciseItem.sets.length">
            <li
              v-for="setItem in exerciseItem.sets"
